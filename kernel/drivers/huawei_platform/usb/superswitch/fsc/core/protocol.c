...
#include "vdm.h"
//TODO: Pass timer to vdm function... or something
//#include "typec.c" /* TODO: temp for debug */

void FUSB3601_USBPDProtocol(struct Port *port)
{
  /* Received hard reset? */
  if ((port->registers_.AlertL.I_RXHRDRST)
		  || (port->registers_.AlertL.I_TXSUCC && port->registers_.AlertL.I_TXFAIL)){
    FUSB3601_ClearInterrupt(port, regALERTL, MSK_I_RXHRDRST | MSK_I_TXSUCC | MSK_I_TXFAIL);
    ...
    if(port->registers_.AlertL.I_RXHRDRST) FUSB3601_WritePDToken(&port->log_, FALSE, pdtHardReset);
#endif /* FSC_LOGGING */
  }
  else {

	  if (port->registers_.AlertL.I_RXSTAT && !port->protocol_msg_rx_) {
		  //platform_log(port->port_id_, "FUSB Rx Detected", -1);
		...
		//platform_log(port->port_id_, "FUSB Clear I_RXSTAT", -1);
		...
		//WritePEState(&port->log_, port->registers_.AlertL.byte, port->policy_state_);
		//WritePEState(&port->log_, port->registers_.AlertH.byte, port->policy_state_);

//	    if((port->policy_rx_header_.NumDataObjects !=0) &&
//	    		(port->policy_rx_header_.MessageType == DMTVendorDefined)) {
//	    	//platform_log(port->port_id_, "FUSB Begin Additional VDM Check", -1);
//	    	platform_delay(2 * 1000);
//
//	    	ReadRegister(port, regALERTL);
//	    	if (port->registers_.AlertL.I_RXSTAT) {
//	    		//platform_log(port->port_id_, "FUSB VDM Interruption Detected - New Rx", -1);
...
void FUSB3601_ProtocolGetRxPacket(struct Port *port)
{
  ...
  //WritePEState(&port->log_, 200, port->policy_state_);
  ...
  FUSB3601_WritePDMsg(&port->log_, port->policy_rx_header_,
		  port->policy_rx_data_obj_, FALSE, rx_sop);

  /* Store the (recreated) GoodCRC message that we have sent (SOP) */
  FUSB3601_WritePDMsg(&port->log_, temp_header,
             0, TRUE, rx_sop);
#endif /* FSC_LOGGING */
...
void FUSB3601_ProtocolTransmitMessage(struct Port *port)
{
  ...
  //platform_log(port->port_id_, "FUSB Tx Message", -1);
...
/* JIRA - 297 Test */
  if((temp_PolicyTxHeader.NumDataObjects !=0) &&
  	    		(temp_PolicyTxHeader.MessageType == DMTVendorDefined)) {
  ...
  FUSB3601_WritePDMsg(&port->log_, temp_PolicyTxHeader,
             port->policy_tx_data_obj_, TRUE, port->protocol_msg_tx_sop_);
#endif /* FSC_LOGGING */
...
void FUSB3601_ProtocolSendingMessage(struct Port *port)
{
  ...
  if (port->registers_.AlertL.I_TXSUCC) {
    ...
    FUSB3601_WritePDMsg(&port->log_, temp_header,
	  ...
	  if(port->ams) FUSB3601_set_policy_state(port, port->fail_state);
	  else if(port->source_or_sink_ == Source) FUSB3601_set_policy_state(port, peSourceReady);
	  else if(port->source_or_sink_ == Sink) FUSB3601_set_policy_state(port, peSinkReady);
...
void FUSB3601_PolicySendVDM(struct Port *port, Command command, FSC_U16 svid, SopType sop, FSC_U8 obj_pos) {
	...
	if(port->source_or_sink_ == Source) fail_state = peSourceSendSoftReset;
	else fail_state = peSinkSendSoftReset;
