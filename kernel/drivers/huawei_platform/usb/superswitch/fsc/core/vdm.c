...
//          RequestEnterMode(port, SOP_TYPE_SOP, SVID_AUTO_ENTRY,
//                           port->display_port_data_.AutoModeEntryObjPos);
...
//      }
	...
	//TODO:adjust length for ama, set correct softreset for power role
	FUSB3601_ProtocolSendData(port, DMTVendorDefined, 4, vdm_out, 0, peSinkSendSoftReset, FALSE, sop);
  }
  else {
...
// Need a callback that informs that a vdm request failed
	//port->current_vdm_request
	//platform_vdm_fail(port, command, nak/busy/timeout)
	port->auto_vdm_state_ = AUTO_VDM_DONE;
	port->expecting_vdm_response_ = FALSE;
}
