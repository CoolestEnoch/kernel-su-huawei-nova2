...
#define SHORT_TEST_PASS "-4P"
#define SHORT_TEST_FAIL "-4F"
#define SELFCAP_TEST_PASS "-6P"
#define SELFCAP_TEST_FAIL "-6F"
#define RESISTANCE_TEST_PASS "-8P"
#define RESISTANCE_TEST_FAIL "-8F"
#define TD43XX_EE_SHORT_TEST_FAIL "-4F"
#define TD43XX_EE_SHORT_TEST_PASS "-4P"

#define NO_AUTO_CAL_MASK 0x01
#define F54_BUF_LEN 80
#define TP_TEST_FAILED_REASON_LEN 20
#define HYBRID_BUF_SIZE 100
#define MMI_SHORT_BUF_SIZE 7
#define MMI_HIGHRESISTANCE_BUF_SIZE 3
#define HYBRID_SELF_CAP_NUMS 30
#define TEST_PASS 1
#define TEST_FAIL 0
#define EE_SHORT_VALID_DATA 1
#define EE_SHORT_INVALID_DATA 0
...
static void mmi_hybrid_raw_cap_test(void)
{
	unsigned char command;
	int result = 0;
...
static void mmi_hybrid_abs_delta_test(void)
{
	unsigned char command;
	int result = 0;
...
static void mmi_trex_highresistance_test(void)
{
	unsigned char command;
	int result = 0;
...
static int f54_enhance_delta_rx_report(void)
{
	int tx_n, rx_n;
	int rx_delta_value = 0;
	int result = 0;

	for (rx_n = 0; rx_n < f54->rmi4_data->num_of_rx - 1; rx_n++) {
		for (tx_n = 0; tx_n < f54->rmi4_data->num_of_tx; tx_n++) {
			rx_delta_value = abs(
			/* rawdatabuf[0] rawdatabuf[1] are num_of_rx and num_of_tx */
				f54->rawdatabuf[(int)(tx_n * f54->rmi4_data->num_of_rx + rx_n + 2 + 1)] -
				f54->rawdatabuf[(int)(tx_n * f54->rmi4_data->num_of_rx + rx_n + 2)]);
			if (rx_delta_value <= f54->rmi4_data->synaptics_chip_data->rx_delta[(int)(tx_n * (f54->rmi4_data->num_of_rx - 1) + rx_n)]) {
				result++;
			}
		}
	}
	if((mmi_buf_size / 2 - f54->rmi4_data->num_of_tx) == result) {
		return 1;
	} else {
		return 0;
	}
}
static int f54_enhance_delta_tx_report(void)
{
	int tx_n, rx_n;
	...
	for (tx_n = 0; tx_n < f54->rmi4_data->num_of_tx - 1; tx_n++) {
		for (rx_n = 0; rx_n < f54->rmi4_data->num_of_rx; rx_n++) {
			tx_delta_value = abs(
			/* rawdatabuf[0] rawdatabuf[1] are num_of_rx and num_of_tx */
				f54->rawdatabuf[(int)((tx_n + 1) * f54->rmi4_data->num_of_rx + rx_n + 2)] -
				f54->rawdatabuf[(int)(tx_n * f54->rmi4_data->num_of_rx + rx_n + 2)]);
			if (tx_delta_value <= f54->rmi4_data->synaptics_chip_data->tx_delta[(int)(tx_n * f54->rmi4_data->num_of_rx + rx_n)]) {
				result++;
			}
		}
	}
	if((mmi_buf_size / 2 - f54->rmi4_data->num_of_rx) == result) {
		return 1;
	} else {
		return 0;
	}
...
int get_int_average(int *p, size_t len)
{
	int sum = 0;
	size_t i;
...
int get_int_min(int *p, size_t len)
{
	int s_min = SHRT_MAX;
	size_t i;
...
int get_int_max(int *p, size_t len)
{
	int s_max = SHRT_MIN;
	size_t i;
...
static void put_capacitance_short_data(int index)
{
	int i,j,temp;
	for (i = 0, j = index + 2; i < MMI_SHORT_BUF_SIZE; i++, j++) {
		temp = f54->mmi_buf[i];
		f54->rawdatabuf[j] = temp;
	}
}

static void put_capacitance_highresistance_data(int index)
{
	int i, j;
	int k = 0;
	int count = 0;
	...
	f54->rawdatabuf[0] = f54->rmi4_data->num_of_rx;
	f54->rawdatabuf[1] = f54->rmi4_data->num_of_tx;
	for (i = 0, j = index + 2; i < 2 * MMI_HIGHRESISTANCE_BUF_SIZE; i += 2, j++) {
		temp = (f54->mmi_buf[i]) | (f54->mmi_buf[i + 1] << 8);
		f54->rawdatabuf[j] = temp;
		if(i < MMI_HIGHRESISTANCE_BUF_SIZE) {
			if(f54->rawdatabuf[j] < f54->rmi4_data->synaptics_chip_data->high_resistance[k]) {
				count ++;
			}
		} else {
			if(f54->rawdatabuf[j] > f54->rmi4_data->synaptics_chip_data->high_resistance[k]) {
	...
	if(MMI_HIGHRESISTANCE_BUF_SIZE == count) {
		strncat(buf_f54test_result, RESISTANCE_TEST_PASS, MAX_STR_LEN);
	}
	else {
		strncat(buf_f54test_result, RESISTANCE_TEST_FAIL, MAX_STR_LEN);
	}
}

static void selfcap_hybrid_check(void)
{
	int i, j;
	int count = 0;
	for(i = 0; i < HYBRID_SELF_CAP_NUMS; i++)
	{
		if(abs(f54->hybridbuf[i]) < f54->rmi4_data->synaptics_chip_data->self_cap_noise_max) {
			count ++;
		}
	}
	for(j = 0, i = HYBRID_SELF_CAP_NUMS; i < (HYBRID_SELF_CAP_NUMS + HYBRID_SELF_CAP_NUMS); i++, j++)
	{
		if((f54->hybridbuf[i] < f54->rmi4_data->synaptics_chip_data->self_cap_hybrid_upper[j])
			&& (f54->hybridbuf[i] > f54->rmi4_data->synaptics_chip_data->self_cap_hybrid_lower[j])) {
			count ++;
		}
	}

	if((HYBRID_SELF_CAP_NUMS + HYBRID_SELF_CAP_NUMS) == count) {
		strncat(buf_f54test_result, SELFCAP_TEST_PASS, MAX_STR_LEN);
	} else {
		strncat(buf_f54test_result, SELFCAP_TEST_FAIL, MAX_STR_LEN);
	}
}
...
static int td43xx_ee_short_normalize_data(signed short * image, unsigned int lens)
{
	...
	char left_size = TD43XX_EE_SHORT_TEST_LEFT_SIZE;
	char right_size = TD43XX_EE_SHORT_TEST_RIGHT_SIZE;
	...
	signed int temp = 0;
	unsigned int right_data_size = right_size * rx_num;
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			...
			if (temp < part_two_limit) {
				image[i * rx_num + j] = EE_SHORT_VALID_DATA;
			}
			else {
				image[i * rx_num + j] = EE_SHORT_INVALID_DATA;
			}
		}
	}
...
static int td43xx_ee_short_report(void)
{
	...
	for (i = 0, k = 0; i < (long)tddi_ee_short_data_size; i++) {
		td43xx_rt95_part_one[i] = buffer[k] |
					(buffer[(long)k + 1]) << SHIFT_ONE_BYTE;
		td43xx_rt95_part_one[i] = (td43xx_rt95_part_one[i] > part_one_limit) ?
									EE_SHORT_TEST_FAIL : EE_SHORT_TEST_PASS;
		k += NORMAL_NUM_TWO;
	}

	for (i = 0, k = td43xx_rt95_report_size; i < (long)tddi_ee_short_data_size; i++) {
	...
	for (ii = 0; ii < tx_num; ii++) {
		for (jj = 0; jj < rx_num; jj++) {
			td43xx_ee_short_data[ii*rx_num +jj] =
				(td43xx_rt95_part_one[ii * rx_num + jj]) ||
...
int synaptics_get_cap_data(struct ts_rawdata_info *info)
{
	...
	if (rc < 0) {		/*i2c communication failed, mmi result is all failed */
		...
		if (f54->rmi4_data->synaptics_chip_data->tddi_ee_short_test_support) {
		...
		if (f54->rmi4_data->synaptics_chip_data->test_rawdata_normalizing) {
			...
			selfcap_hybrid_check();
			mmi_trex_highresistance_test();
			mmi_buf_size = mmi_buf_size + MMI_SHORT_BUF_SIZE;
			put_capacitance_highresistance_data(mmi_buf_size);
			mmi_trex_shorts_test();
			mmi_buf_size = mmi_buf_size - MMI_SHORT_BUF_SIZE;
			put_capacitance_short_data(mmi_buf_size);
		}
		mmi_add_static_data();
	}
	if (SYNAPTICS_TD4322 != f54->rmi4_data->synaptics_chip_data->ic_type
	...
	memcpy(info->buff, f54->rawdatabuf, (rawdata_size + MMI_SHORT_BUF_SIZE + MMI_HIGHRESISTANCE_BUF_SIZE) * sizeof(int));
	memcpy(info->hybrid_buff, f54->hybridbuf, (mmi_hybrid_abs_delta/2) * sizeof(int));
	...
	if(f54->rmi4_data->synaptics_chip_data->test_rawdata_normalizing) {
		info->used_synaptics_short_size = MMI_SHORT_BUF_SIZE;
...
int synaptics_get_calib_data(struct ts_calibration_data_info *info)
{
	int rc = NO_ERR;
	unsigned char command;
...
int synaptics_get_calib_info(struct ts_calibration_info_param *info)
{
	int rc = NO_ERR;
...
static int synaptics_rmi4_f54_attention(void)
{
	...
	if ((int)(f54->report_size) > mmi_buf_size)
		return NO_ERR;
...
unsigned short synaptics_f54_get_calibrate_addr(struct synaptics_rmi4_data
						*rmi4_data,
						const char *module_name)
{
	int rc = NO_ERR;
