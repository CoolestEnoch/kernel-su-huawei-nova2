...
int synaptics_fw_s3718_configid(struct synaptics_rmi4_data *rmi4_data, u8 *buf,
				size_t buf_size)
{
	int rc = 0;
...
#if 0
		if (properties_2.has_guest_code || properties_2.has_force_config || properties_2.has_lockdown_data
				|| properties_2.has_lcm_data || properties_2.has_oem_data) {
			retval = fwu->fn_ptr->read(rmi4_data,
					base + fwu->off.properties_2 + 1,
					buf,
					count);
			if (retval < 0) {
				TS_LOG_ERR
...
exit:
	//synaptics_fw_data_s3718_release();  //avoid panic
...
static int synaptics_read_project_id(void)
{
	...
	if (fwu->bl_version != BL_V7) {
		if ((fwu->rmi4_data->synaptics_chip_data->ic_type != SYNAPTICS_TD4322)
			&& (fwu->rmi4_data->synaptics_chip_data->ic_type != SYNAPTICS_TD4300)) {
			memcpy(project_id,
...
static int file_open_firmware(u8 **buf)
{
	...
	*buf = vmalloc(length + (length % 2));	/* buf size if even */
	...
	/* read data */
	if ((filp)->f_op->read((filp), (*buf), length, &(filp)->f_pos) !=
	    length) {
 ...
 */
int synaptics_get_fw_data_s3718_boot(char *file_name,
				     struct touch_settings
				     *synaptics_sett_param_regs)
{
	int retval;
	size_t file_name_size =
	    strlen(file_name) + strlen(synaptics_sett_param_regs->module_name);
