...
static int tfa9895_i2c_read(char *rx_data, int length)
{
	int ret = 0;
	struct i2c_msg msg[] = {
		{
...
static int tfa9895_i2c_write(char *rx_data, int length)
{
	int ret = 0;
...
static int tfa9895_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ret) {
		hwlog_err("%s: tfa9895_device register failed (%d)", __func__, ret);
	}
