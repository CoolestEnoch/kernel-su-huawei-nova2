...
#define MAX_FILE_ID	90
#define FP_DELAY_10 10
#define FP_DELAY_5   5
#define SPI_2              2
#define RESET_HIGH   1
#define RESET_LOW    0
...
static struct gyro_platform_data gyro_data = {
	...
	.temp_compens_in_offset = 0,//temperature compensation set to angular speed of uncalib data
};
...
static void read_gyro_data_from_dts(struct device_node *dn)
{
	...
	if (of_property_read_u32(dn, "temp_compens_in_offset", &temp))
		hwlog_info("%s:read temp_compens_in_offset fail\n", __func__);
	else{
		gyro_data.temp_compens_in_offset = (uint8_t) temp;
		hwlog_info("%s:read gyro temp_compens_in_offset %d\n", __func__, temp);
	}
...
static void read_capprox_data_from_dts(struct device_node *dn)
{
	...
	if (of_property_read_u32(dn, "calibrate_type", &temp))
		...
		sar_pdata.calibrate_type=(uint8_t)temp;
	if (of_property_read_u32(dn, "stage_number", &temp))
		hwlog_err("%s:read sar stage_number fail\n", __func__);
	else
	...
	} else if (!strncmp(sensor_chip_info[CAP_PROX], "huawei,adi-double-adux1050", strlen("huawei,adi-double-adux1050"))) {
		init_reg_val = sar_pdata.sar_datas.adux_data.init_reg_val;
		if (of_property_read_u32_array(dn, "init_reg_val", init_reg_val, 16)) {
			hwlog_err("%s:read init_reg_val fail\n", __func__);
			memcpy(init_reg_val, init_reg_val_default, sizeof(init_reg_val_default));
		}
		hwlog_info("init_reg_val:%8x %8x\n", *init_reg_val, *(init_reg_val+15));

                threshold_to_modem = sar_pdata.sar_datas.adux_data.threshold_to_modem;

                if (of_property_read_u16_array(dn, "to_modem_threshold", threshold_to_modem,4)) {
                        *threshold_to_modem =  0xC8;
                        *(threshold_to_modem+1) = 0;
                        hwlog_info("read threshold_to_modem fail\n");
                }
                hwlog_info("read threshold_to_modem %u %u %u\n", *threshold_to_modem, *(threshold_to_modem+1), *(threshold_to_modem+
3));
		...
		if (of_property_read_u32_array(dn, "init_reg_val", init_reg_val, 12)) {
			hwlog_err("%s:read init_reg_val fail\n", __func__);
			memcpy(init_reg_val, init_reg_val_default, sizeof(init_reg_val_default));
		}
		hwlog_info("init_reg_val:%8x %8x %8x\n", *init_reg_val, *(init_reg_val+5),*(init_reg_val+11));
...
static void read_rpc_data_from_dts(struct device_node* dn)
{
    int temp = 0;
    int i;
    u32 wia[32]={ 0, };
    ...
    if (of_property_read_u32(dn, "default_value", &temp))
...
static int fingerprint_sensor_detect(struct device_node *dn, int index, struct sensor_combo_cfg *cfg)
{
	...
	if (!ret) {
		if (!strncmp(sensor_vendor, "fpc", LEN_3)) {
			gpio_direction_output(gpio_reset, RESET_HIGH);
			msleep(FP_DELAY_10);
			hwlog_info("%s: fingerprint device %s\n", __func__, sensor_vendor);
		} else if(!strncmp(sensor_vendor, "goodix", LEN_6)) {
			gpio_direction_output(gpio_reset, RESET_HIGH);
			msleep(FP_DELAY_10);
			gpio_direction_output(gpio_reset, RESET_LOW);
			msleep(FP_DELAY_10);
			gpio_direction_output(gpio_reset, RESET_HIGH);
			...
			tx[0] = 0xC0;
			tx_len = 1;
			ret = mcu_spi_rw(SPI_2, ctrl, tx, tx_len, NULL, LEN_0); /* set sensor to idle mode */
			if (ret != 0)
			{
				hwlog_err("%s: mcu_spi_rw set sensor idle, ret = %d\n", __func__, ret);
			}
			tx[0] = 0xF0;
			tx[1] = 0x00;
			tx[2] = 0x00;
			tx_len = 3;
			ret = mcu_spi_rw(SPI_2, ctrl, tx, tx_len, NULL, LEN_0); /* write cmd & address */
			if (ret != 0)
			{
				hwlog_err("%s: mcu_spi_rw write cmd, ret = %d\n", __func__, ret);
			}
			hwlog_info("%s: fingerprint device %s\n", __func__, sensor_vendor);
		} else if(!strncmp(sensor_vendor, "silead", LEN_6)) {
			gpio_direction_output(gpio_reset, RESET_HIGH);
			msleep(FP_DELAY_10);
			gpio_direction_output(gpio_reset, RESET_LOW);
			msleep(FP_DELAY_10);
		...
		if (ret != 0)
		{
			hwlog_err("%s: _device_detect failed, ret = %d\n", __func__, ret);
		}
	} else {
...
int init_sensors_cfg_data_from_dts(DETECT_MODE mode)
{
	...
	if (s_detect_manager[FINGERPRINT].detect_result != DET_SUCC)
	{
		hwlog_warn("fingerprint detect fail, bypass.\n");
		s_detect_manager[FINGERPRINT].detect_result = DET_SUCC;
	}
	    ...
	    if (s_id == RPC)
            {
                ...
                hwlog_err("send rpc data to mcu sucess! default_value = %d,mask[0] =%d\n",rpc_data.default_value,rpc_data.mask[0]);
