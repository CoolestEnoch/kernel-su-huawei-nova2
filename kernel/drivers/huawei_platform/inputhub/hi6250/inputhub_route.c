...
#define DSM_BATTERY_NUM 70
#define DSM_USB_SHORT_PROTECT_NUM 5
static uint32_t charge_event;
static int need_upload = 0;
extern struct work_struct   g_intb_work_sh;
...
extern int notify_sensorhub(uint8_t stype, uint8_t rw);
extern void update_charging_info(struct charge_device_info_sh *di);
extern void update_coul_sensorhub_info(void);
extern int coul_get_battery_capacity(void);
extern int charge_usb_notifier_call_sh(struct notifier_block *usb_nb,
				    unsigned long event, void *data);
extern void dsm_uscp_report(uint32_t uscp_id);
extern int sensorhub_charger_dsm_report(int err_no, int *val);
#endif
						  ...
						  close_param_t *param)
{
	...
	if ((TAG_SENSOR_BEGIN <= tag && tag < TAG_SENSOR_END) || (tag == TAG_AOD)) {
		if (enable) {
			pkt_header_t pkt = {
				.tag = tag,
...
#ifdef CONFIG_HUAWEI_CHARGER_SENSORHUB
static void enable_charing_when_recovery_iom3(void)
{
...
#ifdef CONFIG_HUAWEI_CHARGER_SENSORHUB
	enable_charing_when_recovery_iom3();
#endif
	...
	if ((TAG_SENSOR_BEGIN <= tag && tag < TAG_SENSOR_END) || (tag == TAG_AOD)) {
		if (enable) {
			...
			hwlog_info("close sensor %s (tag:%d)!\n",
				   obj_tag_str[tag] ? obj_tag_str[tag] :
				   "TAG_UNKNOWN", tag);
		      if(tag == TAG_RPC && rpc_motion_request == 1){
			  	rpc_motion_request = 0;
                        }
			hwlog_info("rpc_motion_request val = %d", rpc_motion_request);
                        return inputhub_mcu_write_cmd_adapter(&pkt, sizeof(pkt),
							      NULL);
		}
	} else {
	...
	if (NULL == rd) {
		...
		while (retry_count--) {	/*send retry 3 times*/
			...
			if (inputhub_mcu_write_cmd(buf, length)) {
				...
				hwlog_err
				    ("wait for response timeout in %s, retry %d. tag %d cmd %d. g_iom3_state %d.\n",
				     __func__, retry_count,
				     ((pkt_header_t *) buf)->tag,
				     ((pkt_header_t *) buf)->cmd, g_iom3_state);
				if (retry_count == 0) {
...
#ifdef CONFIG_HUAWEI_CHARGER_SENSORHUB
static bool is_charging_events_data_report(const pkt_header_t *head)
{
	return (TAG_CHARGER == head->tag)
	    && (CMD_CHARGER_DATA_REPORT_REQ == head->cmd);
}
#endif
...
#ifdef CONFIG_HUAWEI_CHARGER_SENSORHUB
static void iom7_charging_event_handle(struct work_struct *work)
{
	struct iom7_charging_work *iom7_charging =
	    container_of(work, struct iom7_charging_work, charging_work);
	...
	if(charge_event == SWITCH_INTB_WORK_EVENT) {
		schedule_work(&g_intb_work_sh);
		return;
	}

	hwlog_info("iom7_charging_event_handle, event is %d\n", charge_event);
	if (iom3_power_state != ST_SLEEP) {
		if(USB_EVENT_OTG_ID != pConfigOnDDr->g_di.charger_type)
			hisi_coul_charger_event_rcv(charge_event);
		else
			hwlog_err("otg mode ignore event!!\n");
		update_charging_info(&pConfigOnDDr->g_di);
		update_coul_sensorhub_info();
		notify_sensorhub(CHARGE_UPDATE_CHARGE_INFO, 0);
	} else
		need_upload = 1;
...
#ifdef CONFIG_HUAWEI_CHARGER_SENSORHUB
	if ((pkt->dmd_id >= DSM_BATTERY_ERROR_NO_BASE) && (pkt->dmd_id <= (DSM_BATTERY_ERROR_NO_BASE+DSM_BATTERY_NUM)))
		sensorhub_charger_dsm_report(pkt->dmd_id, NULL);
	else if ((pkt->dmd_id >= ERROR_NO_USB_SHORT_PROTECT) && (pkt->dmd_id <= (ERROR_NO_USB_SHORT_PROTECT+DSM_USB_SHORT_PROTECT_NUM)))
		dsm_uscp_report(pkt->dmd_id);
	else
#endif
#ifdef SENSOR_DSM_CONFIG
	if (!dsm_client_ocuppy(shb_dclient)) {
		hwlog_info("dsm_client_ocuppy\n");
