...
static void read_mag_data_from_dts(struct device_node *dn)
{
	...
	if (of_property_read_u32(dn, "charger_trigger", &temp))
		...
		hwlog_info("%s:read mag charger_trigger = %d",__func__,
...
static void read_capprox_data_from_dts(struct device_node *dn)
{
	...
	if (of_property_read_u16(dn, "flag_for_threshold_config", &temp)){
		hwlog_err("%s: read flag_for_threshold_config fail\n", __func__);
		sar_pdata.flag_for_threshold_config = 0;
		}
	else{
		sar_pdata.flag_for_threshold_config = (uint16_t) temp;
		hwlog_err("%s: read flag_for_threshold_config %d ok\n", __func__,sar_pdata.flag_for_threshold_config);
	}

	if (of_property_read_u32(dn, "calibrate_delay", &temp)){
		hwlog_err("%s: read calibrate_delay fail\n", __func__);
		sar_pdata.calibrate_delay = 0;
	}
	else{
		sar_pdata.calibrate_delay = (uint16_t) temp;
		hwlog_err("%s: read calibrate_delay %d ok\n", __func__,sar_pdata.calibrate_delay);
	}
	...
	if (!strncmp(sensor_chip_info[CAP_PROX], "huawei,cypress_sar_psoc4000", strlen("huawei,cypress_sar_psoc4000"))) {
		...
		if (of_property_read_u16_array(dn, "to_modem_threshold", threshold_to_modem, 8)) {
			hwlog_err("%s: read to_modem_threshold fail\n", __func__);
			*threshold_to_modem =  0;
			*(threshold_to_modem+1) = 0;
		}
