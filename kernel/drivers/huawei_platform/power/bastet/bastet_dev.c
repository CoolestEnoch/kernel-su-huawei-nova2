...
extern int bastet_comm_write(u8 *msg, u32 len, u32 type);
...
extern int bastet_send_msg(struct bst_sock_id *guide, uint8_t *data, uint32_t len);
extern int bastet_set_hb_reply(struct bst_sock_id *guide, uint8_t *data, uint32_t len);
...
static long bastet_ioctl(struct file *flip, unsigned int cmd, unsigned long arg)
{
	...
	case BST_IOC_SET_TRAFFIC_FLOW: {
		...
		if (copy_from_user(&flow_p, argp,
			sizeof(struct bst_traffic_flow_pkg)))
			break;
		...
		if (copy_from_user(buf, argp
			+ sizeof(struct bst_traffic_flow_pkg), buf_len)) {
	...
	case BST_IOC_BUF_AVAILABLE: {
		...
		BASTET_LOGI("buffer_available=%d",
			is_buffer_available());
		rc = 0;
		break;
	}
	case BST_IOC_PRIO_SEND_SUC: {
		...
		BASTET_LOGI("channel_available=%d",
	...
	case BST_IOC_SYNC_HOLD_TIME: {
		...
		if (copy_from_user(&hold_delay,
			argp, sizeof(hold_delay)))
			break;
	...
	case BST_IOC_UID_PROP_MONITOR: {
		...
		if (copy_from_user(&prop, argp,
			sizeof(struct app_monitor_prop))) {
			rc = -EFAULT;
			break;
		}
		...
		if (prop.tid_count > 0) {
			...
			if (copy_from_user(tids,
				argp + sizeof(struct app_monitor_prop),
	...
	case BST_IOC_SET_PROC_INFO: {
		...
		if (copy_from_user(&info, argp,
			sizeof(struct set_process_info))) {
			rc = -EFAULT;
			break;
		}
	...
	case BST_IOC_GET_SOCK_NETDEV_NAME: {
	/*
		 * get network device name of socket for usersapce
		 */
		struct get_netdev_name ndev_name;

		if (copy_from_user(&ndev_name,
			argp, sizeof(struct get_netdev_name)))
			break;
		...
		if (copy_to_user(argp,
			&ndev_name, sizeof(struct get_netdev_name)))
			rc = -EFAULT;
		break;
	}
	case BST_IOC_SEND_HB_DATA: {
		/*
		...
		if (copy_from_user(&send,
			argp, sizeof(struct heartbeat_content))) {
			rc = -EFAULT;
			break;
		}
		/*
		 ...
		 */
		if (copy_from_user(content,
			argp + sizeof(struct heartbeat_content), send.len)) {
			kfree(content);
			rc = -EFAULT;
			break;
		}
	...
	case BST_IOC_SET_HB_REPLY: {
		...
		content = (uint8_t *) kmalloc(reply.len, GFP_ATOMIC);
		if (NULL == content) {
		 ...
		 */
		if (copy_from_user(content,
			argp + sizeof(struct heartbeat_content), reply.len)) {
			kfree(content);
			rc = -EFAULT;
			break;
		}
	...
	case BST_IOC_FILTER_HB_REPLY: {
		...
		if (copy_from_user(&guide, argp, sizeof(struct bst_sock_id))) {
			rc = -EFAULT;
			break;
		}
		/*
	...
	case BST_IOC_SET_FREEZER_STATE: {
		/*
		 * set process freeze state to struct sock
		 */
		struct freezer_state freezer;

		if (copy_from_user(&freezer,
			argp, sizeof(struct freezer_state))) {
			rc = -EFAULT;
			break;
		}
		/*
	...
	case BST_IOC_SET_PRIO_CH_ENABLE: {
		int32_t enable = 0;
...
static int bastet_packet_read(char __user *buf, size_t count)
{
	...
	len = sizeof(struct bst_device_ind)
		- sizeof(pkt->data.cons) + pkt->data.len;
 ...
 */
/*lint -e666*/
static ssize_t bastet_read(struct file *filp,
