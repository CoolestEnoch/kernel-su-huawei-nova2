...
static int bq25892_set_bat_comp(int value)
{
	...
	if (bat_comp < BAT_COMP_MIN_0) {
		hwlog_info("set term current %dmA is out of range:%dmA!!",
	...
	return bq25892_write_mask(BQ25892_REG_08, BQ25892_REG_08_BAT_COMP_MASK,
...
static int bq25892_set_vclamp(int value)
{
	...
	return bq25892_write_mask(BQ25892_REG_08, BQ25892_REG_08_VCLAMP_MASK,
...
static int bq25892_set_covn_start(int enable)
{
	...
	ret =
	    bq25892_write_mask(BQ25892_REG_02, BQ25892_REG_02_CONV_START_MASK,
	...
	for (i = 0; i < 10; i++) {
		ret =
		    bq25892_read_mask(BQ25892_REG_02,
...
static int bq25892_set_dpm_voltage(int value)
{
	...
	if (vindpm_voltage > VINDPM_MAX_15300) {
		hwlog_info("set dpm voltage %d mv is out of range:%dmA!!",
...
static int bq25892_set_terminal_current(int value)
{
	...
	return bq25892_write_mask(BQ25892_REG_05, BQ25892_REG_05_ITERM_MASK,
...
static int bq25892_set_otg_current(int value)
{
	...
	return bq25892_write_mask(BQ25892_REG_0A, BQ25892_REG_0A_BOOST_LIM_MASK,
...
{
	return bq25892_write_mask(BQ25892_REG_07, BQ25892_REG_07_EN_TERM_MASK,
...
{
	return bq25892_write_mask(BQ25892_REG_03, BQ25892_REG_03_WDT_RESET_MASK,
...
static int bq25892_get_ico_current(struct ico_output *output)
{
	...
	if (ret) {
		hwlog_err("bq25892_read_byte BQ25892_REG_09 fail\n");
		return ret;
	}
	...
	if (ret) {
		hwlog_err("bq25892 force ico fail\n");
		return ret;
	}
	...
	while (check_ico_optimized_times--) {
		ret = bq25892_read_byte(BQ25892_REG_14, &reg);
		if (ret) {
			hwlog_err("bq25892_read_byte BQ25892_REG_14 fail\n");
			return ret;
		}
		if (reg & BQ25892_REG_14_ICO_OPTIMIZED_MASK) {
			...
			if (ret) {
				hwlog_err("bq25892_read_byte BQ25892_REG_13 fail\n");
				return ret;
			}
		...
		hwlog_info("bq25892_start_force_ico:  msleep\n");
		msleep(200);
	}
...
static int bq25892_turn_on_ico(struct ico_input *input, struct ico_output *output)
{
	...
	if(ret) {
		hwlog_err("bq25892_set_charge_enable fail.\n");
		return -1;
	}
	ret = bq25892_set_input_current(input->iin_max);
	if(ret) {
		hwlog_err("bq25892_set_input_current fail.\n");
		return -1;
	}
	ret = bq25892_set_charge_current(input->ichg_max);
	if(ret) {
		hwlog_err("bq25892_set_charge_current fail.\n");
		return -1;
	}
	ret = bq25892_set_terminal_voltage(input->vterm);
	if(ret) {
		hwlog_err("bq25892_set_terminal_voltage fail.\n");
		return -1;
	}
...
static void bq25892_irq_work(struct work_struct *work)
{
	struct bq25892_device_info *di =
	    container_of(work, struct bq25892_device_info, irq_work);
	u8 reg = 0, reg1 = 0;
...
void prase_dts(struct device_node *np, struct bq25892_device_info *di)
{
	...
	ret = of_property_read_u32
		(np, "ico_current_mode", &ico_current_mode);
	if (ret) {
	...
	if (of_property_read_u32
	    (np, "adc_channel_vbat_sys", &adc_channel_vbat_sys)) {
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (BAT_BOARD_UDP != is_board_type) {
		...
		ret =
		    request_irq(di->irq_int, bq25892_interrupt,
