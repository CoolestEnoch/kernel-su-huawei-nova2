...
HWLOG_REGIST();
#else
#define hwlog_debug(fmt, args...)do { printk(KERN_DEBUG   "[hisi_battery_data]" fmt, ## args); } while (0)
	...
	for (i = 0; i < pdat->pc_temp_ocv_lut0->cols; i++) {	/* 6 */
		for (j = 0; j < pdat->pc_temp_ocv_lut0->rows; j++) {	/* 29 */
			if(pdat->vol_dec1 > 0)
			{
				...
				if (ret) {
					hwlog_err("get pc_temp_ocv_ocv1[%d] failed\n", j * TEMP_SAMPLING_POINTS + i);
				}
			}
			if(pdat->vol_dec2 > 0)
			{
				...
				if (ret) {
					hwlog_err("get pc_temp_ocv_ocv2[%d] failed\n", j * TEMP_SAMPLING_POINTS + i);
				}
			}
			if(pdat->vol_dec3 > 0)
			{
				...
				if (ret) {
					hwlog_err("get pc_temp_ocv_ocv3[%d] failed\n", j * TEMP_SAMPLING_POINTS + i);
				}
			}
		}
	}
