...
static struct attribute *hi6522_sysfs_attrs[ARRAY_SIZE(hi6522_sysfs_field_tbl) +
					    1];
...
**********************************************************/
static struct hi6522_sysfs_field_info *hi6522_sysfs_field_lookup(const char
								 *name)
{
	int i, limit = ARRAY_SIZE(hi6522_sysfs_field_tbl);
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	if (!strncmp
	    ("dpm_switch_enable", attr->attr.name,
...
{
	u8 Iin_limit;
	if(HIZ_IIN_FLAG_TRUE == hiz_iin_limit_flag){
...
static int hi6522_set_charge_current(int charge_current)
{
	...
	Ichg_limit =
	    (unsigned char)((charge_current - CHG_FAST_ICHG_MIN) /
...
static int hi6522_set_terminal_current(int term_current)
{
	...
	Iterm =
	    (unsigned char)((term_current - CHG_TERM_ICHG_MIN) /
...
{
	return hi6522_write_mask(CHG_FAST_SAFE_TIMER_TERM_CTRL_REG,
				 CHG_FAST_SAFE_TIMER_MSK,
				 CHG_FAST_SAFE_TIMER_SHIFT,
...
/****************************************************************************
  ...
  Description:  config chopper enable/disable.
  Input:        struct hi6522_device_info,di->wled need setup.
  Output:       NA
  Return:       0:success others:fail
****************************************************************************/
...
/****************************************************************************
  ...
  Description:  config ifb1_ifb2 enable/disable.
  Input:        struct hi6522_device_info,di->wled need setup.
  Output:       NA
  Return:       0:success others:fail
****************************************************************************/
...
static int hi6522_config_flash_torch_timeout(struct hi6522_device_info *di)
{
	...
	tout =
	    (unsigned char)((flash_torch_timeout - CHG_FLASH_TORCH_TIMOUT_MIN) /
			    CHG_FLASH_TORCH_TIMOUT_STEP_2);
...
int scharger_power_status(int id)
{
	...
	case SCHG_LDO3_CPN1_ID:	//vsp && vsn:
		ret = (scharger_di->power_on_config_reg00.reg.lcd_ldo_en_int
		       && scharger_di->power_on_config_reg00.reg.
		       lcd_ncp_en_int);
		break;
	default:
		break;
...
/* return ok when charger probe later than regulator*/
	...
	if (down_interruptible(&di->charger_data_busy_lock)) {
		SCHARGER_ERR
		    ("scharger_power_get_current_limit get sema fail! \n");
		return -EINTR;
	}
	/*get voltage */
...
/* return ok when charger probe later than regulator*/
	...
	if (down_interruptible(&di->charger_data_busy_lock)) {
		SCHARGER_ERR
		    ("scharger_power_set_current_limit get sema fail! \n");
		return -EINTR;
	}
	/*set current */
	ret = hi6522_read_byte((u8) vset_regs, &reg_value);
...
/**********************************************************
*  Function:        hi6522_get_vbus_mv
*  Discription:     get vbus
*  Parameters:     none
*  return value:   -1:fail;  other: vbus
**********************************************************/
static int hi6522_get_vbus_mv(unsigned int *vbus_mv)
{
	...
	/*SchargerV200 do not support get-vbus function, for CTS, we return a fake vulue */
	*vbus_mv = 5000; //fake voltage value for CTS
...
static int hi6522_device_check(void)
{
    ...
    if(CHIP_VER_V2XXX == reg_chip_ver){
...
/**********************************************************
*  Function:        hi6522_check_input_dpm_state
...
static int hi6522_check_input_dpm_state(void)
{
    ...
    if (ret < 0)
    {
        SCHARGER_ERR("hi6522_check_input_dpm_state err\n");
        return ret;
    }

    if (CHG_STATUS0_BUCK_DPM_STAT == (reg & CHG_STATUS0_BUCK_DPM_STAT))
    {
        return TRUE;
    }
    else
...
module_param(optimize_delay_ms, int, 0644);
static int ico_hig_thr_count = 0;
static int batteryBufferFirst = 1;
static int batteryVoltageBuffer[VBATT_AVR_MAX_COUNT] = {0};

/**********************************************************
*  Function:        mt_battery_average_method_init
*  Discription:     battery average voltage initialize
*  Parameters:      *bufferdata:buffer to store data
*                   data:data to store
*                   *sum:the total result of the data
*  return value:    NULL
**********************************************************/
...
/**********************************************************
*  Function:        mt_battery_average_method
*  Discription:     battery average voltage computation
*  Parameters:      *bufferdata:buffer to store data
*                   voltage:battery voltage
*  return value:    average battery volage
**********************************************************/
...
/**********************************************************
*  Function:        hi6522_is_dpm_state_valid
*  Discription:     check whether dpm state is valid
*  Parameters:      NULL
*  return value:    TRUE means VINDPM
*                   FALSE means NoT DPM
**********************************************************/
...
/****************************************************************
*  Function:        voltage_base_input_current_select
*  Discription:     select iin current by battery current
*  Parameters:      iin_option_num:input current option,
*                   *di:hi6522_device_info
*  return value:    NULL
******************************************************************/
static void voltage_base_input_current_select(int iin_option_num,struct hi6522_device_info *di)
{
    static int index = 0;
    unsigned int iin_lim = 0;
    unsigned int state = 0;
    int ret = 0;

    hi6522_set_input_current(IIN_LIMIT_500mA);
    hi6522_set_charge_current(SCHARGER_ICHG_MAX);//unlock charge current limit
    hi6522_set_charge_enable(TRUE);
    msleep(optimize_delay_ms);
    for(index = iin_option_num; index < IIN_OPTION_NUM; index++)
    {
        iin_lim = iin_limit_option[index];
        hi6522_set_input_current(iin_lim);
        if(TRUE == hi6522_is_dpm_state_valid())
        {
            break;
        }
    }
...
/****************************************************************
*  Function:        hi6522_input_current_optimize
*  Discription:     choose the suitable input current for charger
*  Parameters:      *input:input parameters,
*                   *output:parameters to take out
*  return value:    return 0
******************************************************************/
static int hi6522_input_current_optimize(struct ico_input *input, struct ico_output *output)
{
    struct hi6522_device_info *di = g_hi6522_dev;
...
static void hi6522_dpm_check_work(struct work_struct *work)
{
	...
	if ((current_ma > dpm_check_current_threshold_ma)
		...
		if (dpm_switch_with_charge_stop) {
			if (down_interruptible(&di->charger_data_busy_lock)) {
				SCHARGER_ERR("hi6522_dpm_check_work get sema fail! \n");
			}
...
/**********************************************************
*  Function:       scharger_flash_bst_vo_config
*  Discription:    Interface for modifing flash boost voltage
*  Parameters:     config_voltage:target voltage
*  return value:   0-sucess or others-fail
**********************************************************/
int scharger_flash_bst_vo_config(int config_voltage)
{
	...
	di = scharger_di;
	if (down_interruptible(&di->charger_data_busy_lock)) {
		SCHARGER_ERR("scharger_flash_bst_vo_config get sema fail! \n");
		return -EINTR;
	}
	...
	Boost_vol =
	    (unsigned char)((FLASH_BST_VOL_MAX - config_voltage) /
			    FLASH_BST_VOL_STEP_100);
	ret =
	    hi6522_write_mask(FLASH_BST_REG10_REG, FLASH_BST_VOL_MSK,
...
EXPORT_SYMBOL_GPL(scharger_flash_bst_vo_config);
...
static void hi6522_irq_work_handle(struct work_struct *work)
{
	...
	if (irq_status[0] & CHG_IRQ0_VBUS_TRICKLE_CHG_FAULT) {
		SCHARGER_ERR
		    ("SCHARGER ERROR:TRICKLE_CHG_FAULT!,reg[0x07]:0x%x.\n",
		     irq_status[0]);
	}
	...
	if (irq_status[1] & CHG_IRQ1_WLED_OVP_4P5V) {
		...
		hi6522_config_wled_enable(di);
		SCHARGER_ERR
		    ("SCHARGER ERROR:CHG_IRQ1_WLED_OVP_4P5V,reg[0x08]:0x%x.\n",
		     irq_status[1]);
		atomic_notifier_call_chain(&fault_notifier_list,
					   CHARGE_FAULT_SCHARGER,
					   NULL);
...
{
	return blocking_notifier_chain_register(&lcd_ldo_ocp_init_notifier_list,
						nb);
}
...
{
	return
	    blocking_notifier_chain_unregister(&lcd_ldo_ocp_init_notifier_list,
					       nb);
}
					   ...
					   unsigned long event, void *data)
{
	...
	case CHARGER_TYPE_NONE:
		...
		/*In LPT, to avoid batfet is auto opened when plugout usb cable, disable it again*/
		if((strstr(saved_command_line, "androidboot.swtype=factory"))
		    &&(di->batfet_disable_flag == TRUE)){
		    msleep(1000);
		    hi6522_set_batfet_ctrl(!(di->batfet_disable_flag));
		}
		break;
...
static void parse_dts(struct device_node *np, struct hi6522_device_info *di)
{
    ...
    ret = of_property_read_u32( np,"input_current_limit_option",&(di->ico_iin_lim_opt));
    if(ret)
    {
				...
				const struct i2c_device_id *id)
{
	...
	ret =
	    request_irq(di->irq_int, hi6522_irq_handle,
			IRQF_TRIGGER_LOW | IRQF_NO_SUSPEND, di->client->name,
			di);
	if (ret) {
...
/* scharger v200 need to do dpm check when usb/charger is plugged on startup*/
	...
	if (power_class) {
		if (charge_dev == NULL) {
			charge_dev =
			    device_create(power_class, NULL, 0, "%s",
					  "charger");
