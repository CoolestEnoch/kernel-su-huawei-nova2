    ...
    size_t off, struct hilog_entry *scratch)
{
    ...
    if (len != sizeof(struct hilog_entry)) {
        memcpy(((void *) scratch), dev->buffer + off, len); /* unsafe_function_ignore: memcpy */  // copy from the tail of buffer.
        memcpy(((void *) scratch) + len, dev->buffer, sizeof(struct hilog_entry) - len); /* unsafe_function_ignore: memcpy */  // copy from the head of buffer.
        return scratch;
    }
...
static ssize_t do_write_hilog(struct hilog_device *dev, const void *buf, size_t count)
{
    ...
    memcpy(dev->buffer + dev->w_off, buf, len); /* unsafe_function_ignore: memcpy */

    if (count != len) {
        memcpy(dev->buffer, buf + len, count - len); /* unsafe_function_ignore: memcpy */
    }
...
static ssize_t create_hilog_dev(void)
{
    ...
    memset(dev, 0, sizeof(struct hilog_device)); /* unsafe_function_ignore: memset */
                           ...
                           const char* fmt, ...)
{
    ...
    va_start(ap, fmt);
    vsnprintf(msg, DEFAULT_FORMATTED_LEN, fmt, ap); /* unsafe_function_ignore: vsnprintf */
    va_end(ap);
