...
#else
const uint32_t nvme_crc32c_table[256] = {
...
uint32_t crc32c_nve(uint32_t crc, const uint8_t *p, unsigned long long len)
{
	while (len--)
		crc = nvme_crc32c_table[(crc ^ *p++) & 0xff] ^ (crc >> 8);
	return crc;
}
