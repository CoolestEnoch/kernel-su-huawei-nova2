...
static void novatek_chip_touch_switch(void){
	...
	*ptr_end = 0;
	...
	if (TS_SWITCH_TYPE_DOZE == stype &&
		...
		switch (soper) {
			case TS_SWITCH_GAME_ENABLE:
				...
				buf[0] = 0x51;
				buf[1] = 0x1;
				error = novatek_ts_kit_i2c_write(nvt_ts->client, I2C_FW_Address, buf, 2);
				if (error < 0) {
				...
				buf[0] = 0x5A;
				buf[1] = 0x0;
				error = novatek_ts_kit_i2c_read(nvt_ts->client, I2C_FW_Address, buf, 2);
				if (error < 0) {
	...
	} else if (TS_SWITCH_TYPE_SCENE <= stype &&
		TS_SWITCH_TYPE_SCENE == (nvt_ts->chip_data->touch_switch_flag & TS_SWITCH_TYPE_SCENE)) {
