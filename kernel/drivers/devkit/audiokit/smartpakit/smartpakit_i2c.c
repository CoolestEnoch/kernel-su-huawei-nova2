		...
		for (i = 0; i < (int)sequence->num; i++) {
			...
			if (SMARTPAKIT_REG_CTL_TYPE_W == ctl_type) {
				...
				if (i2c_priv->irq_handler->compare_value_existed) {
					...
					hwlog_info("%s: pa[%d], r reg[0x%x] = 0x%x, compare_value = 0x%x\n", __func__, i2c_priv->chip_id, reg_addr, value, compare_value);
					if ((value & ctl_value) != compare_value) {
						hwlog_info("%s: pa[%d], reg[0x%x] = 0x%x,regs value abnormal, need reset pa\n", __func__, i2c_priv->chip_id, reg_addr, value);
...
static int smartpakit_i2c_read(struct i2c_client *i2c, char *rx_data, int length)
{
	...
	if (0 > ret) {
		smartpakit_dsm_report_by_i2c_error(0xff, 0, ret);
...
static int smartpakit_i2c_write(struct i2c_client *i2c, char *rx_data, int length)
{
	...
	ret = i2c_transfer(i2c->adapter, msg, 1);
	if (0 > ret) {
		smartpakit_dsm_report_by_i2c_error(0xff, 1, ret);
		hwlog_err("%s: transfer error %d", __func__, ret);
		return ret;
	}
	...
	for (i = 0; i < (int)pakit_priv->pa_num; i++) {
		...
		if (pakit_priv->i2c_priv[i]->irq_handler->need_reset) {
			if (pakit_priv->i2c_priv[i]->irq_handler->compare_value_existed) {
				check = pakit_priv->i2c_priv[i]->irq_handler->irq_need_reset_status ? true : false;
...
static int smartpakit_i2c_parse_dt_irq(struct i2c_client *i2c, smartpakit_i2c_priv_t *i2c_priv)
{
	...
	if (of_property_read_bool(node, rw_sequence_str)) {
		...
		if (ret < 0) {
			...
			irq_handler->rw_sequence->compare_value = kzalloc(sizeof(unsigned int) * irq_handler->rw_sequence->num, GFP_KERNEL);
			if (NULL == irq_handler->rw_sequence->compare_value) {
				hwlog_err("%s: kzalloc reg_ctl_sequence compare_value failed!!!\n", __func__);
				ret = -EFAULT;
				goto err_out;
			}
			if (of_property_read_bool(node, "rw_sequence_compare")) {
				count = of_property_count_elems_of_size(node, "rw_sequence_compare", (int)sizeof(u32));
				if (count != irq_handler->rw_sequence->num) {
					irq_handler->compare_value_existed = false;
					hwlog_err("%s: rw_sequence_compare num and rw_sequence num not equal\n", __func__);
					goto err_out;
				} else {
					ret = of_property_read_u32_array(node, "rw_sequence_compare", (u32 *)irq_handler->rw_sequence->compare_value, (size_t)(long)count);
					if (ret < 0) {
						irq_handler->compare_value_existed = false;
						hwlog_err("%s: reg_ctl get compare_value failed!!!\n", __func__);
						goto err_out;
					}
				}
				...
				irq_handler->compare_value_existed = false;
				hwlog_info("%s: rw_sequence_compare prop not existed, free compare_value, skip!!!\n", __func__);
			}
		}
	} else {
