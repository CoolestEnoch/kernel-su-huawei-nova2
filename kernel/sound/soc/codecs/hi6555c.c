...
#define CLASSK_GPIO_STATE_SWITCH_DELAY   2 //units:us
#define CLASSK_GPIO_NUM_DEFAULT      -1
#define CLASSK_ENABLE_WORKQUENE_DELAY_TIME    100 //units:ms
...
static void hi6555c_misc_reg_write(struct work_struct *work)
{
	...
	switch (reg) {
	case HI6555C_CLASSD_POWER_IF:
		if (!priv->use_battery_supply && !gpio_is_valid(priv->spk_pa_en_gpio)) {
			...
			if(gpio_is_valid(priv->spk_pa_en_gpio)) {
				if (HI6555C_ENABLE == value) {
					logi("%s:pull up gpio of controling classd power!", __FUNCTION__);
					gpio_set_value(priv->spk_pa_en_gpio, GPIO_PULL_UP);
				} else {
					logi("%s:pull down gpio of controling classd power!", __FUNCTION__);
					gpio_set_value(priv->spk_pa_en_gpio, GPIO_PULL_DOWN);
				}
			} else {
...
static inline unsigned int hi6555c_reg_read(struct snd_soc_codec *codec, unsigned int reg)
{
	...
	if (INVALID_REG_VALUE == ret) {
		loge("reg 0x%pK read value 0x%x is invalid\n", (void *)(unsigned long)reg, ret);
	}
 ...
 *
 *****************************************************************************/
static int classk_mode_set(int mode)
{
	int i;
	if (classk_en_gpio < 0) {
		loge("classK_en_gpio not exist!!\n");
		return -ENOMEM;
	}
	//this action is to power down classK before classK-mode change
	gpio_direction_output(classk_en_gpio, GPIO_PULL_DOWN);
	udelay(CLASSK_MODE_CHANGE_DELAY);

	//set classK work mode by pull down and pull up gpio alternately
	for (i = 0; i < mode; i++)
	{
		gpio_direction_output(classk_en_gpio, GPIO_PULL_DOWN);
		udelay(CLASSK_GPIO_STATE_SWITCH_DELAY);
		gpio_direction_output(classk_en_gpio, GPIO_PULL_UP);
		udelay(CLASSK_GPIO_STATE_SWITCH_DELAY);
	}

	logi("set classK mode %d sucecess!!!\n", mode);
	return 0;
}

static const char * const classk_mode_text[] = {"MODE0", "MODE1", "MODE2", "MODE3", "MODE4", "MODE5",
                                                "MODE6", "MODE7", "MODE8", "MODE9", "MODE10"};

static const struct soc_enum classk_mode_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(classk_mode_text), classk_mode_text),
};

static int classk_mode_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	if (NULL == kcontrol || NULL == ucontrol) {
		loge("input pointer is null\n");
		return 0;
	...
	ucontrol->value.integer.value[0] = classk_mode;

	return 0;
}

static int classk_mode_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	int ret = 0;
	...
	classk_mode = ucontrol->value.integer.value[0];

	logi("classk_mode is %d\n", classk_mode);
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((0 != ucontrol->value.integer.value[0]%100)
		...
		||(ucontrol->value.integer.value[0]>CLASSD_5500mV_VOL)){
...
static const struct snd_kcontrol_new hi6555c_snd_controls[] = {
	...
	SOC_SINGLE("HI6555C_SMT_BUCK3_VSET",
			HI6555C_SMT_BUCK3_VSET, HI6555C_SMT_BUCK3_VSET_BIT_START, 0xf, 0),
	...
	SOC_SINGLE("HISSC_PMU_CLASSD_CTRL1_I_PUMP",
			HI6555C_PMU_CODEC_CLASSD_CTRL1_ADDR, HI6555C_PMU_CLASSD_CTRL1_I_PUMP_BIT, 0x3, 0),

	SOC_SINGLE("HISSC_PMU_CLASSD_CTRL1_P_SEL",
			HI6555C_PMU_CODEC_CLASSD_CTRL1_ADDR, HI6555C_PMU_CLASSD_CTRL1_P_SEL_BIT, 0x3, 0),

	SOC_SINGLE("HISSC_PMU_CLASSD_CTRL1_N_SEL",
			HI6555C_PMU_CODEC_CLASSD_CTRL1_ADDR, HI6555C_PMU_CLASSD_CTRL1_N_SEL_BIT, 0x3, 0),
		...
		struct snd_kcontrol *kcontrol, int event)
{
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		hi6555c_reg_write(g_codec, HI6555C_SMT_CODEC_ANA_RW17_ADDR, 0x50);
		hi6555c_reg_write(g_codec, HI6555C_SMT_CODEC_ANA_RW18_ADDR, 0x50);
		break;
	case SND_SOC_DAPM_POST_PMD:
		break;
	default:
...
static int hi6555c_spk_power_mode_event(struct snd_soc_dapm_widget *w,
				struct snd_kcontrol *k, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		queue_delayed_work(priv->classk_enable_dwq, &priv->classk_enable_dw,
				msecs_to_jiffies(CLASSK_ENABLE_WORKQUENE_DELAY_TIME));
		break;
	case SND_SOC_DAPM_PRE_PMD:
		cancel_delayed_work(&priv->classk_enable_dw);
		flush_workqueue(priv->classk_enable_dwq);
...
static void hi6555c_reuse(struct snd_soc_codec *codec)
{
	...
	hi6555c_set_reg_bits(HI6555C_SW_RST_N_ADDR,  HI6555C_CODEC_16BIT_SETTING);
...
static const char *hi6555c_extern_classk_switch_texts[] = {
	"OFF", "ON",
};

static const struct soc_enum hi6555c_extern_classk_switch =
SOC_ENUM_SINGLE(SND_SOC_NOPM, 0,
		ARRAY_SIZE(hi6555c_extern_classk_switch_texts), hi6555c_extern_classk_switch_texts);

static const struct snd_kcontrol_new hi6555c_extern_classk_switch_dapm_controls =
SOC_DAPM_ENUM("SWITCH", hi6555c_extern_classk_switch);
...
static const struct snd_soc_dapm_widget hi6555c_dapm_widgets[] = {
		...
		SND_SOC_DAPM_SPK("SPK", hi6555c_spk_power_mode_event),
		...
		SND_SOC_DAPM_MUX("CLASSK SWITCH",
				SND_SOC_NOPM,
				0,
				0/* not INVERT */,
				&hi6555c_extern_classk_switch_dapm_controls),
...
static const struct snd_soc_dapm_route hi6555c_route_map[] = {
	...
	{"LM_CODEC_DAC2ADC MUX",               "CIC_DACL",              "SIF_ADCL_EN PGA"},
	{"RM_CODEC_DAC2ADC MUX",               "CIC_DACR",             "SIF_ADCR_EN PGA"},
...
static inline int hi6555c_irq_stat(unsigned int irq_stat_bit)
{
	...
	irq_state = hi6555c_reg_read(g_codec, HI6555C_SMT_ANA_IRQ_SIG_STAT_ADDR);
...
static void hs_plug_in_detect_func(struct snd_soc_codec *codec)
{
	 ...
	 */
	if ((hkadc_value < HS_MAX_VOLTAGE) && (priv->pre_status_is_lineout == false)) {
...
{
	struct hi6555c_priv *priv = snd_soc_codec_get_drvdata(codec);
	logi("hs_plug_out_detect_func enter\n");
	BUG_ON(!priv);
...
static int hs_btn_down_detect_func(struct snd_soc_codec *codec)
{
	...
	if (HI6555C_JACK_BIT_NONE != priv->pressed_btn_type) {
...
void hi6555c_lineout_rm_recheck_work_func(struct work_struct *work)
{
	struct snd_soc_codec *codec;
	struct hi6555c_priv *priv = container_of(work, struct hi6555c_priv, lineout_po_recheck_dw.work);/*lint !e826*/
	int hkadc_value;

	BUG_ON(!priv);/*lint !e730*/
	BUG_ON(!priv->codec);/*lint !e730*/
	...
	mutex_lock(&priv->plug_mutex);
	if(!hi6555c_irq_stat(IRQ_STAT_PLUG_IN)) {
		hi6555c_irqs_clr(IRQ_MSK_COMP);/*lint !e845*/
		hi6555c_irqs_mask_clr(IRQ_PLUG_IN);
		mutex_unlock(&priv->plug_mutex);
	...
	if (hkadc_value < HS_MAX_VOLTAGE) {
		/*btn_up event*/
		mutex_unlock(&priv->plug_mutex);
		return;
	} else {
		/*report plugout event,and set hs_status to lineout mode*/
		if (priv->hs_status == HI6555C_JACK_BIT_LINEOUT) {
			mutex_unlock(&priv->plug_mutex);
			...
			hi6555c_irqs_clr(IRQ_MSK_COMP);/*lint !e845*/
			hi6555c_irqs_mask_clr(IRQ_MSK_BTN_NOR);/*lint !e845*/
...
static void hs_lineout_rm_recheck_func(struct snd_soc_codec *codec)
{
	struct hi6555c_priv *priv = snd_soc_codec_get_drvdata(codec);
...
bool hs_lineout_plug_out_func(struct hi6555c_priv *priv)
{
	...
	if (hkadc_value < HS_MAX_VOLTAGE) {
		/*btn_up event*/
		mutex_unlock(&priv->plug_mutex);
		hs_lineout_rm_recheck_func(codec);
		return false;
	} else {
		/*report plugout event,and set hs_status to lineout mode*/
		mutex_lock(&priv->io_mutex);
		...
		if (0 != priv->pressed_btn_type) {
			...
			/*report key event*/
			logi("report type=0x%x, status=0x%x\n", priv->hs_jack.report, priv->hs_status);
			snd_soc_jack_report(&priv->hs_jack.jack, priv->hs_jack.report, JACK_RPT_MSK_BTN);/*lint !e655*/
		}
		priv->pressed_btn_type = 0;
		mutex_unlock(&priv->io_mutex);
...
static void hi6555c_hs_comp_h_btn_down_work_func(struct work_struct *work)
{
	...
	if(hs_lineout_plug_out_func(priv)) {
		logi("hs_lineout_plug_out, return .\n");
		return;
	}
...
static void set_headset_keys_config(struct hi6555c_priv *priv)
{
	...
	priv->headset_voltage.hs_4_pole_max_voltage = 2650;
...
static int hi6555c_soc_init(struct snd_soc_codec *codec)
{
	...
	if (!priv->use_battery_supply) {
		...
		if (IS_ERR(priv->regulator_schg_boost3)) {
			...
			ret = (int)PTR_ERR(priv->regulator_schg_boost3);
			goto modem0_clk_enabled;
		}
	}
...
regulator_enabled:
	if (regulator_disable(priv->asp_cfg_regu) != 0) {
...
int hi6555c_irq_config(struct hi6555c_priv *priv)
{
	int ret = 0;
	...
	INIT_DELAYED_WORK(&priv->lineout_po_recheck_dw, hi6555c_lineout_rm_recheck_work_func);/*lint !e747*/
...
static void hi6555c_spk_pa_init(struct snd_soc_codec *codec)
{
	int ret = 0;
	int value = -1;
	struct hi6555c_priv *priv = NULL;
	...
	priv->spk_pa_en_gpio = value;
	ret = of_property_read_u32(np, "gpio_spk_pa_enable", (u32*)&value);
	if(0 != ret)
	{
		logi("%s:get gpio_spk_pa_enable failed!\n", __FUNCTION__);
		return;
	}

	priv->spk_pa_en_gpio = value;

	if (gpio_request(priv->spk_pa_en_gpio, "spk_pa_en_gpio")) {
		loge("%s:gpio_request failed!\n", __FUNCTION__);
		return;
	}

	if (gpio_direction_output(priv->spk_pa_en_gpio, GPIO_PULL_DOWN)) {
		loge("%s:gpio_direction_output failed!\n", __FUNCTION__);
		return;
	}
	return;
}

static void hi6555c_spk_pa_uninit(struct snd_soc_codec *codec)
{
	struct hi6555c_priv *priv = NULL;
	...
	if(gpio_is_valid(priv->spk_pa_en_gpio))
	{
		logi("%s:gpio_free priv->spk_pa_en_gpio.\n", __FUNCTION__);
		gpio_free(priv->spk_pa_en_gpio);
	}
	return;
}
...
{
	logi("in: mode is %d\n", classk_mode);
	classk_mode_set(classk_mode);
}

static int hi6555c_extern_classk_init(struct snd_soc_codec *codec)
{
	struct hi6555c_priv *priv = NULL;
	struct device_node *np  = codec->dev->of_node;

	if (of_property_read_u32(np, "hisilicon,classK_gpio", (u32*)&classk_en_gpio)) {
		logi("classk is not support.\n");
		return 0;
	}

	if (gpio_request(classk_en_gpio, "classK_en_gpio")) {
		classk_en_gpio = CLASSK_GPIO_NUM_DEFAULT;
		loge("request for gpio resources failed.\n");
		return -EFAULT;
	} else {
		gpio_direction_output(classk_en_gpio, GPIO_PULL_DOWN);
	}
	...
	priv->classk_enable_dwq = create_singlethread_workqueue("classk_enable_dwq");
	if (NULL == priv->classk_enable_dwq) {
		loge("classK_enable_dwq workqueue create failed\n");
		return -EFAULT;
	}

	INIT_DELAYED_WORK(&priv->classk_enable_dw, hi6555c_extern_classk_enable_func);

	return 0;
}

static void hi6555c_extern_classk_uninit(struct snd_soc_codec *codec)
{
	struct hi6555c_priv *priv = NULL;
	...
	if (NULL != priv->classk_enable_dwq) {
		cancel_delayed_work(&priv->classk_enable_dw);
		flush_workqueue(priv->classk_enable_dwq);
		destroy_workqueue(priv->classk_enable_dwq);
		priv->classk_enable_dwq = NULL;
	}

end:
	if (classk_en_gpio >= 0) {
		gpio_free(classk_en_gpio);
	}
...
static void hi6555c_free_resource(struct snd_soc_codec *codec)
{
	...
	hi6555c_spk_pa_uninit(codec);
	hi6555c_extern_classk_uninit(codec);
